...
(async function ver4_ExtractEmailSmsActivities() {
  ...
  for (const el of nodes) {
    ...
    if (!parent) continue;
    const dataNode = parent.getAttribute('data-node') || '';
    // match EDM or SMS at the start of the data-node value
  ...
  const header = ['Activity Name', 'Type'];
  const rows = found.map(f => [f.name, f.type].map(v => v.replace(/\t/g,' ')).join('\t'));
  ...
  if (ok) alert(`Copied ${found.length} EDM/SMS activity(ies) to clipboard (TSV). Paste into Google Sheets A1.`);
  else alert('Copy failed — TSV printed to console.');
  console.log('ver4 TSV:\n' + tsv);
  console.table(found.map(f => ({ name: f.name, type: f.type, dataNode: f.dataNode })));
  return found;
})();
